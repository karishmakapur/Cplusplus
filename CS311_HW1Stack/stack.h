...
public: // prototypes to be used by the client
 ...
 // exception handling classes  
	...
	//          otherwise, throws an exception - Underflow
	// PARAMETER: povide variable to receive the top element without removing it. (pass by reference)
	void topElem(el_t&);

	// ** Must add good comments for each function - See Notes1B

	//PURPOSE: checks to see if the stack is empty.
	//if stack if empty, then the function returns true. Else it returns false.
	// PARAMETER: No parameters are needed
	bool isEmpty();

	//PURPOSE: checks to see if the stack is full.
	//if stack is full, then the function returns true. Else it returns false.
	// PARAMETER: No parameters are needed
	bool isFull();

	//PURPOSE: displays all elements in the stack to user without removing it from the stack.
	// PARAMETER: No parameters are needed
	void displayAll();

	//PURPOSE: clears the stack by removing each element one at a time until stack is empty.
	// PARAMETER: No parameters are needed
	void clearIt();
